...
the timestamps to more friendly format.
...
Let's start warming up to try different machine learning models, by first
experimenting with a simple model that always return the average of the ratings 
observed in the training set as prediction.

This model is described by the following equation:
$$
Y_{u,m} = \mu + \varepsilon_{u,m}
$$
Where $Y_{u,m}$ is the rating given by the user $u$ to the movie $m$, $\mu$ is the average of the observed ratings, and $\varepsilon_{u,m}$ is the independent error (variability) of the prediction of the rating for the user $u$ to the movie $m$.


The following function creates the object to represent this model:
...
And the following table displays the performance result for the model:
...
## Linear Like Model based on movie and user effects
