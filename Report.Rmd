...
I.e. the purpose of is not to retrieve a list of recomended movies given an user,
instead is predicting the rating given a set of pairs movie-user.
...
In this report different different methods are being developed, tested and evaluated,
...
and just used for the performance's evaluation of the methods.
...
As a general observation can be seen that the performance in terms of RMSE has been 
...
With higher accuracies that the model based on the movies and users effects,
but the RMSEs not enough to reach the goal of this projects.
...
$R_{U \times M}$ -- being $U$ the number of users and $M$ the number of movies --
as the product of two matrices of lower dimension $P_{U \times K}$ and 
$Q_{M \times K}$ -- being $K$ a given number of factors --. 
...
they can not be completly fill the matrix $R$, so missing values can be filled
...
The following constructor function creates an object to repesent this model,
in this case a number of factors $K$ of 30 is used.
...
It can be observed that the 4 best results corresponds to the two methods
...
ones based on ratings' frequency, but the differency is small among the first 8
...
because they consider better the "impulsive" ratings by the users. 
...
The script that provides the final solution (called `solution.R`)
...
however the training of this model involves some randomization which might
cause the retrieved RMSE to slighly vary (from the forth decimal position).
