...
As a general observation can be seen that the performance in terms of RMSE was 
improved in comparison to the model based on the ratings' mode,
however the accuracy went worse,
either way the performance is still bellow the one desired for this project.
...
global error given by metrics like the RSMSE.
...
It can be observed that the accuracy was increased in comparison to the previous models,
however the RMSE presents a poorer performance than the one based on 
the movie and user effets, non of the RMSEs is not enought for the objective
of this project.
...
rater than staying consistent in their ratings 
...
It can be observed that both the RMSEs and accuracies are very similar to the ones
