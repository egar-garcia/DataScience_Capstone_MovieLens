...
object-oriented approach is going to be used, each method is going to be represented 
...
Then this model can be used to make predictions, like in the following code predicting
agains the training and the validation sets:
...
are used, at first impression looks like half start ratings are not very popular among users.
...
using a small subset of the dataset, since using the whole one might take a while.
...
were allowed after certain point in time, and before that just full-stars were allowed.

To find the point in time were rating ending in half star started to appear, the following code can be used. In basically gets the minimum timestamp in the dataset
...
Converting `half_stars_startpoint` to a more redable representation, 
by executing `as_datetime(half_stars_startpoint)`, the point in time when half-star
ratings started to appear was `r as_datetime(half_stars_startpoint)`.
